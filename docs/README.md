...
> 실서버(핀셋)와 베타(AiBT)는 동일 코드베이스이며, `VITE_APP_BRAND`·`VITE_FEATURE_COUPON` 환경변수로 구분합니다.
...
|------|-----------|----------------------------|
| [04_정책_유저플로우.md](./04_정책_유저플로우.md) | 서비스 목적, 사용자 여정, 라우트 정의. | **실서버 기준** 전체 플로우: 비로그인(무료 진단→20문항→로그인 유도), 로그인 무료(1·2회차만), 로그인 유료(전 회차+맞춤형). 라우트 `/`, `/mypage`, `/exam-list`, `/quiz`, `/result`, `/checkout`, `/admin` 등이 **무엇을 보여주는지** 정의. 예창패 사업계획서용. |
| [02_정책_결제.md](./02_정책_결제.md) | 결제·장학금·이용권 정책. | PG 연동, 회차별 이용권, 할인/환급 등 **Phase별 정책**이 정리됨. 실제 결제 연동은 아직 미진행일 수 있음. |
| [03_정책_회원관리.md](./03_정책_회원관리.md) | 가입·탈퇴·접근 제어. | 누가 어떤 화면/기능에 접근할 수 있는지, 탈퇴 시 데이터 처리 등 **회원 관련 정책**. |
| [FINAL_POLICY_SUMMARY.md](./FINAL_POLICY_SUMMARY.md) | 정책 요약. | 위 정책 문서들의 요약본. 빠르게 전체 정책을 훑을 때 사용. |
...
| 문서 | 내용 요약 | 다른 에이전트를 위한 설명 |
|------|-----------|----------------------------|
| [BETA_유저플로우.md](./BETA_유저플로우.md) | **베타에서 진입 → 로그인 → 쿠폰 → 맞춤형 모의고사까지** 전체 플로우. | 비로그인 시 "로그인 후 이용해 주세요"만 표시·로그인 모달 강제. 구글 로그인만 노출. 로그인 성공 시 **쿠폰 등록 여부** 확인 → 이미 있으면 스킵, 없으면 쿠폰 입력(또는 OT 5페이지). 쿠폰 없으면 오리엔테이션 팝업(forced) 노출. **관련 코드**: `App.tsx`(isBeta·showLoginModal·showOrientationPopup·hasCoupon), `LoginModal.tsx`(구글 로그인·쿠폰 단계·hasCoupon 스킵), `OrientationPopup.tsx`(5페이지 쿠폰·onCouponRegistered). |
