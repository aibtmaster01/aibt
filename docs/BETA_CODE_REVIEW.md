...
**다른 AI 에이전트를 위한 설명**: 베타 판별은 `src/config/brand.ts`의 `APP_BRAND`·`FEATURE_COUPON`(환경변수)으로 이루어지며, `App.tsx`에서 `isBeta` 분기로 로그인 모달 강제·OT 강제·hasCoupon 분기 등을 처리합니다. 상세 유저 플로우는 **BETA_유저플로우.md**를 참고하세요.
