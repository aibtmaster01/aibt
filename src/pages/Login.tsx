...
interface LoginProps {
  /** 처음 진입 시 보여줄 탭 (상단 "시작하기" → 'signup') */
  initialMode?: 'login' | 'signup';
  onBack: () => void;
  onAuthSuccess?: (options?: { isNewUser?: boolean }) => void;
}

export const Login: React.FC<LoginProps> = ({ initialMode = 'login', onBack, onAuthSuccess }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#edf1f5] p-5">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl grid md:grid-cols-2 overflow-hidden min-h-[600px]">
        {/* Left Side (Brand) */}
        <div className="bg-slate-900 p-10 flex flex-col justify-between relative overflow-hidden text-white">
           <div className="absolute top-0 right-0 w-64 h-64 bg-[#0034d3] rounded-full blur-[80px] opacity-20 translate-x-1/2 -translate-y-1/2"></div>
           <div className="relative z-10">
             ...
             <h2 className="text-4xl font-black mb-6 leading-tight">합격으로 가는<br/>가장 빠른 길,<br/>합격해</h2>
             <ul className="space-y-4 text-slate-300">
                <li className="flex items-center gap-3"><span className="text-[#0034d3]">✓</span> 모든 시험 기록 저장 및 관리</li>
                <li className="flex items-center gap-3"><span className="text-[#0034d3]">✓</span> 취약점 분석을 통한 정답률 관리</li>
                <li className="flex items-center gap-3"><span className="text-[#0034d3]">✓</span> 틀린 문제만 모아보는 오답노트</li>
             </ul>
           </div>
           <p className="text-xs text-slate-500">© 2024 합격해. All Rights Reserved.</p>
        </div>

        {/* Right Side (Form) */}
        <div className="p-10 flex flex-col justify-center relative">
          <button onClick={onBack} className="absolute top-6 left-6 text-slate-400 hover:text-slate-900"><ArrowLeft /></button>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button 
              ...
              className="w-full bg-[#0034d3] text-white font-bold py-4 rounded-xl hover:bg-[#003087] transition-colors shadow-lg mt-4 disabled:opacity-50 disabled:cursor-not-allowed"
            >
          ...
          <div className="mt-6 text-center text-sm">
            {mode === 'login' ? (
              <>
                ...
                <button type="button" onClick={() => { setMode('signup'); setError(''); }} className="font-bold text-[#003087] hover:underline">
                  회원가입
                </button>
              </>
            ) : (
              <>
                ...
                <button type="button" onClick={() => { setMode('login'); setError(''); }} className="font-bold text-[#003087] hover:underline">
                  로그인
                </button>
              </>
            )}
