...
export const MyPageBeta: React.FC<MyPageBetaProps> = ({
  ...
  const [activeCertId, setActiveCertId] = useState<string>(() =>
    (typeof initialCertId === 'string' && initialCertId)
      ? initialCertId
  ...
  /** 결과 화면에서 ?cert= 로 진입 시 지연 로드용 (의존 배열 길이 고정으로 React 경고 방지) */
  const initialCertIdRef = useRef(initialCertId);
  initialCertIdRef.current = initialCertId;
  /** 결과에서 왔는데 데이터가 비었을 때 재시도 횟수 (최대 2회, 2회차 저장 지연 대응) */
  ...
  const loadMyPageData = useCallback((forceRefresh?: boolean) => {
    ...
    setLoading(true);
    setLoadError(false);
    // 통계/학습기록과 자격증 정보를 분리: getCertificationInfo 실패 시에도 학습기록은 유지
    getCachedOrFetchMyPageData(user.id, certCode, { forceRefresh })
      ...
      .catch(() => {
        if (loadIdRef.current !== loadId) return;
        // 실패 시 기존 데이터 유지 (새로고침/네트워크 오류로 빈 화면 방지)
        setLoadError(true);
      })
      .finally(() => {
    ...
    getCertificationInfo(certCode)
      .then((info) => setCertInfo(info ?? null))
  ...
  // 결과에서 왔는데 로드 결과가 비어 있으면 최대 2회 재시도 (2회차 Firestore 반영 지연 대응)
  useEffect(() => {
    if (!loading && trendData != null && initialCertIdRef.current) {
      const isEmpty = (trendData.trendData?.length ?? 0) === 0 && (dashboardStats?.radarData?.length ?? 0) === 0;
      if (isEmpty && emptyRetryRef.current < EMPTY_RETRY_MAX) {
        const retryCount = emptyRetryRef.current + 1;
        emptyRetryRef.current = retryCount;
        const delayMs = retryCount === 1 ? 600 : 1100;
        const t = setTimeout(() => loadMyPageData(true), delayMs);
        return () => clearTimeout(t);
      }
    }
  }, [loading, trendData, dashboardStats, loadMyPageData]);
  ...
  const recentPassRate = trendData?.recentPassRate ?? null;
  const diagnosticProgress = trendData?.diagnosticProgress;
  const encouragementMessage = trendData?.encouragementMessage ?? '';
              ...
              {!hasLearningHistory ? (
                ...
                <>
                  <div className="flex-1 flex flex-col items-center justify-center w-full">
                    <p className="text-[#1e56cd] text-4xl md:text-5xl font-black mb-2">
                      {diagnosticProgress?.completed ?? 0}<span className="text-2xl md:text-3xl font-bold text-gray-600">/3</span>
                    </p>
                    <p className="text-slate-600 text-sm text-center mb-4">실력진단 모의고사 완료</p>
                        ...
                        {(displaySubjectScores.length ? displaySubjectScores : freeSubjectScoresForDisplay).slice(0, 4).map((s) => {
                          ...
                          return (
                            <div key={s.subjectNumber} className="flex items-center gap-3 w-full">
                              ...
                              <div className="flex-[0.9] min-w-0 h-5 bg-slate-200 rounded-full overflow-hidden">
                                <div
                                  className="h-full rounded-full transition-all min-w-[2px] bg-[#1e56cd]"
                                  style={{ width: `${Math.min(99, s.score)}%` }}
                                />
                              </div>
                              <span
                                className={`text-xs font-bold shrink-0 px-2 py-1 rounded-md ${
                                  zone === '안정권'
