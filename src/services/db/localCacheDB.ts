...
/** Index는 Firestore에서만 로드 (Storage CORS 회피) */
const USE_FIRESTORE_INDEX_ONLY = FEATURE_COUPON || APP_BRAND === 'AiBT';

/** 베타 BIGDATA 레벨드 인덱스 캐시 키 (certifications/BIGDATA/public/index_leveled 기반) */
...
/**
 * IndexedDB에 저장된 index 캐시 조회. BIGDATA는 레벨드 인덱스 캐시(BIGDATA_beta 키) 사용.
 */
...
/**
 * 해당 자격증 인덱스 캐시만 삭제. BIGDATA는 BIGDATA_beta 키 삭제.
